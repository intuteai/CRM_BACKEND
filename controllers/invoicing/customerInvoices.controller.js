const { CustomerInvoice } = require('../../models/invoicing/customerInvoice');
...
exports.create = async (req, res) => {
  try {
    ...
    if (!invoice_number) return res.status(400).json({ error: 'Invoice number is required', code: 'INVALID_INPUT' });
    const invoice = await CustomerInvoice.create({ customer_id, order_id, invoice_number, issue_date, total_value, link_pdf }, req.io);
    ...
    if (error.code === '23503') return res.status(400).json({ error: 'Invalid customer_id or order_id', code: 'FOREIGN_KEY_VIOLATION' });
...
exports.update = async (req, res) => {
  try {
    ...
    const invoice = await CustomerInvoice.update(req.params.id, { customer_id, order_id, invoice_number, issue_date, link_pdf }, req.io);
    const keys = await redis.keys('invoice_*');
    if (keys.length > 0) await redis.del(keys);
