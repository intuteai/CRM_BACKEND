const DispatchTracking = require('../../models/dispatch/dispatchTracking');
...
exports.update = async (req, res) => {
  ...
  if (!trackingId) return res.status(400).json({ error: 'Tracking ID is required', code: 'INVALID_INPUT' });
  try {
    const updatedDispatch = await DispatchTracking.update(trackingId, { order_id, docket_number, dispatch_date, delivery_date, status });
    if (!updatedDispatch) return res.status(404).json({ error: 'Dispatch tracking entry not found', code: 'NOT_FOUND' });
...
exports.delete = async (req, res) => {
  ...
  try {
    ...
    if (!deleted) return res.status(404).json({ error: 'Dispatch tracking entry not found', code: 'NOT_FOUND' });
    await redis.del('dispatch_tracking_*');
    req.io.emit('dispatchUpdate');
    res.status(204).send();
  } catch (error) {
