const IAOrders = require('../../models/dispatch/iaOrders');
...
function listKey({ limit, cursor }) {
  return cursor ? `${PREFIX}list_${limit}_${encodeURIComponent(cursor)}` : `${PREFIX}list_${limit}_initial`;
}
...
async function invalidate() {
  ...
  } catch (e) { logger.warn(`ia_orders cache invalidation skipped: ${e.message}`); }
}

async function cacheGet(key) {
  try { const r = await redis.get(key); return r ? JSON.parse(r) : null; } catch { return null; }
}

async function cacheSet(key, value) {
  try { await redis.setEx(key, CACHE_TTL, JSON.stringify(value)); } catch (e) { logger.warn(`ia_orders cache set failed: ${e.message}`); }
}
...
exports.create = async (req, res) => {
  try {
    ...
    const order = await IAOrders.create({ customer_name, invoice_number, dispatch_date, notes, items }, req.io);
...
exports.update = async (req, res) => {
  try {
    const { customer_name, invoice_number, dispatch_date, notes, items } = req.body;
    const order = await IAOrders.update(req.params.id, { customer_name, invoice_number, dispatch_date, notes, items }, req.io);
    setImmediate(() => invalidate());
    ...
    return res.status(err.message === 'Order not found' ? 404 : 400).json({ error: err.message });
  }
};
