...
router.get(
  '/',
  authenticateToken,
  checkPermission('Orders', 'can_read'),
  async (req, res, next) => {
    const { limit = 10, cursor, force_refresh = false } = req.query;

    try {
      const parsedLimit = Math.min(parseInt(limit, 10), 100);
      const cacheKey = `orders_${parsedLimit}_${cursor || 'null'}_${
        req.user.user_id
      }`;
      ...
      const itemsByOrder = orderIds.length
        ? await Order.getItemsByOrderIds(orderIds)
        : {};
      ...
      logger.error(
        `Error fetching orders for user ${req.user.user_id}`,
  ...
  async (req, res, next) => {
    try {
      ...
      if (!items?.length || !orderUserId) {
        return res
          .status(400)
          .json({ error: 'Invalid input: items and user_id required' });
      }
      ...
      const order = await Order.create(
        orderUserId,
        req.user.role_id,
        { target_delivery_date: targetDeliveryDate, items },
        req.io
      );
      const itemsDetails = await Order.getItemsByOrderIds([
        order.order_id,
      ]);
      const customer = await pool.query(
        'SELECT name FROM users WHERE user_id = $1',
      ...
      const totalAmount = (itemsDetails[order.order_id] || []).reduce(
        (sum, item) =>
          sum + Number(item.price) * Number(item.quantity),
        0
      );
      ...
      logger.error(`Error in POST /api/orders: ${error.message}`, {
        stack: error.stack,
      });
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          error: `Invalid payment_status: must be ${validPaymentStatuses.join(
            ', '
          )}`,
        });
      }
      ...
      logger.error(
        `Error in PUT /api/orders/${req.params.id}/update: ${error.message}`,
        { stack: error.stack }
      );
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      logger.error(
        `Error in POST /api/orders/${req.params.id}/cancel: ${error.message}`,
        { stack: error.stack }
      );
      res.status(400).json({ error: error.message });
    }
