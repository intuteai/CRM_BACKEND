...
// // --- NEW VALIDATION: productId *or* productName, materials with materialId *or* materialName
// const validateBOM = [
//   body().custom(body => {
...
//       }
//       if (!mat.quantityPerUnit || isNaN(mat.quantityPerUnit) || mat.quantityPerUnit <= 0) {
//         throw new Error('Material quantityPerUnit must be a positive number');
//       }
//     }
//     return true;
//   }),
...
//     }
//     // ...unchanged...
//     // Get logic here (no BOM create/update logic)
//   }
// );
...
//     // Forward both productId/productName and materials to model
//     const { productId, productName, materials } = req.body;
//     logger.info(`Creating BOM`, { productId, productName, materialCount: materials.length });

//     try {
//       const bom = await BOM.create({ productId, productName, materials });
//       await invalidateBOMCache();
//       req.io.emit('bom:created', {
//         ...bom,
//         triggeredBy: req.user.user_id || 'unknown',
...
//         error.message.includes('No price found')
//       ) {
//         res.status(400).json({ error: 'Invalid product/material' });
//       } else {
//         res.status(500).json({ error: 'Failed to create BOM' });
//       }
//     }
...
//     const { id } = req.params;
//     const { productId, productName, materials } = req.body;
//     logger.info(`Updating BOM ${id}`, { productId, productName, materialCount: materials.length });

//     try {
//       const bom = await BOM.update(id, { productId, productName, materials });
//       if (!bom) {
//         logger.info(`BOM ${id} not found`);
//         return res.status(404).json({ error: 'BOM not found' });
//       }
