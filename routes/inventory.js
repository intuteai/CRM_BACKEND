...
// GET /api/inventory
router.get('/', authenticateToken, checkPermission('Inventory', 'can_read'), async (req, res, next) => {
  const { limit = 10, offset = 0, force_refresh } = req.query; // Corrected: No parameter redeclaration
  try {
    const cacheKey = `inventory_${limit}_${offset}`;
    if (force_refresh !== 'true') {
      const cached = await redis.get(cacheKey);
    ...
    const inventory = await Inventory.getAll({ limit: parseInt(limit), offset: parseInt(offset) });
    // Ensure inventory.data includes product_code
    const enrichedInventory = {
      data: inventory.data.map(item => ({
        ...
        product_code: item.product_code || 'N/A', // Ensure product_code is present
      })),
      total: inventory.total,
    };

    await redis.setEx(cacheKey, 3600, JSON.stringify(enrichedInventory));
    logger.info(`Fetched ${enrichedInventory.data.length} inventory items`);
