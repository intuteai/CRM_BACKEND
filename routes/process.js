  ...
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        // Validate work order & process linkage
        const {
          rows: [wo],
        } = await client.query(
        ...
        // Validate aggregate completed qty (excluding this process)
        const {
          rows: [agg],
        } = await client.query(
          'SELECT COALESCE(SUM(completed_quantity), 0) AS total FROM process_status WHERE work_order_id = $1 AND process_id != $2',
        ...
        // If completion_date is provided as empty string, force NULL
      ...
      `;
        ...
        // If there was no row yet, INSERT (UPSERT behavior)
        if (upd.rowCount === 0) {
          const insertSql = `
        ...
        // Recompute work order overall status
        ...
        // Fetch updated WO (use TO_CHAR to avoid Date.toISOString on strings)
        const {
          rows: [uwo],
        } = await client.query(
          `SELECT wo.work_order_id, wo.order_id, wo.component_id, c.component_name, c.product_type,
        ...
        // (cache invalidation & socket emit same as before)
        setImmediate(async () => {
          try {
