...
class DeliveryChallan {
  /**
   * Existing generator that returns a PDFDocument (streaming).
   * Kept for backward compatibility.
   * data.items: [{ source: 'raw'|'inventory', productId, description, qty, remarks, returnable }]
   */
  static generate(data = {}) {
    ...
    try {
      ...
      const gstParty = data.to_gst_number
      ...
      y += 16; // little gap before name

      // Name, address, GST each on own lines below M/s.
      ...
      if (toAddress) {
        doc.text(toAddress, margin, y, {
          width: leftW,
          align: 'left',
        });
        y +=
          doc.heightOfString(toAddress, { width: leftW, align: 'left' }) + 2;
      }

      if (gstParty) {
        doc.text(gstParty, margin, y, {
          width: leftW,
          align: 'left',
        });
      }
      ...
      const metaY = bottomLineY + 8; // align with "To," top
      const rowH = 18;
      const rows = 3;
      const metaH = rowH * rows;

      doc
        .rect(metaX, metaY, rightW - 8, metaH)
        .lineWidth(1)
        .strokeColor(black)
        .stroke();
      ...
      function metaRow(labelLeft, valueLeft, labelRight, valueRight, idx) {
        ...
        // left label + value
        doc.text(labelLeft, metaX + 4, baseY, {
          width: (rightW - 8) / 2 - 8,
          align: 'left',
        });
        doc.text(valueLeft || '', metaX + 4 + 70, baseY, {
          width: (rightW - 8) / 2 - 70,
          align: 'left',
        });

        // right label + value (can be blank)
        if (labelRight) {
          doc.text(labelRight, colMid + 4, baseY, {
            width: (rightW - 8) / 2 - 8,
            align: 'left',
          });
        }
        if (valueRight) {
          doc.text(valueRight || '', colMid + 4 + 60, baseY, {
            width: (rightW - 8) / 2 - 60,
            align: 'left',
          });
        }
      ...
      const challanDateStr = challanDate
        ? challanDate.toLocaleDateString('en-GB')
        : '';
      ...
      // Vehicle row: nothing on right
      metaRow('Vehicle No.:', data.vehicle_no || '', '', '', 2);

      // ========= TEXT ABOVE TABLE (exact wording, above No./Description) =========
      y = metaY + metaH + 16;

      const aboveTableText =
        'Received the under mentioned goods in good & ordered condition.';
      doc
        .font('Roboto')
        .fontSize(9)
        .fillColor(black)
        .text(aboveTableText, margin, y, { width: innerW, align: 'left' });
      ...
      doc.text('No.', tableX + 4, headerY + 4, {
        width: colNoW - 8,
        align: 'center',
      });
      doc.text('Description', tableX + colNoW + 4, headerY + 4, {
        width: colDescW - 8,
        align: 'left',
      });
      doc.text('Qty', tableX + colNoW + colDescW + 4, headerY + 4, {
        width: colQtyW - 8,
        align: 'center',
      });
      doc.text(
        'Remarks',
        tableX + colNoW + colDescW + colQtyW + 4,
        headerY + 4,
        { width: colRemarksW - 8, align: 'left' }
      );
      ...
      items.forEach((it, idx) => {
        // Note: include source & product id in description for clarity (optional)
        const descPieces = [];
        if (it.description) descPieces.push(it.description);
        if (it.source) descPieces.push(`(${it.source}${it.productId ? ` #${it.productId}` : ''})`);
        const desc = descPieces.join(' - ') || '';
      ...
      // ========= TEXT BELOW TABLE (exact paragraph, below rows) =========
      rowY += 10;
      ...
      doc
        ...
        .fontSize(9)
        .fillColor(black)
        .text(belowTableText, margin, rowY, {
          width: innerW,
          align: 'left',
        });

      rowY +=
        doc.heightOfString(belowTableText, {
          width: innerW,
          align: 'left',
        }) + 30; // a bit more gap to footer

      // ========= SIGNATURE AREA =========
      const signY = rowY;

      // left: receiver's signature text
      doc
        .font('Roboto')
        .fontSize(10)
        .fillColor(black)
        .text("Receiver's Signature", margin, signY, {
          width: innerW / 2 - 20,
          align: 'left',
        });
      ...
      doc
        ...
        .fontSize(10)
        .fillColor(black)
        .text(`For ${COMPANY_NAME}`, margin + innerW / 2, signY, {
          width: innerW / 2,
          align: 'right',
        });

      rowY = signY + 40; // more spacious gap before footer

      // ========= REGISTERED OFFICE & FACTORY LINE =========
      const factoryText =
      ...
      doc
        ...
        .fontSize(9)
        .fillColor(black)
        .text(factoryText, margin, rowY, {
          width: innerW,
          align: 'center',
        });

      rowY += 26; // extra space between lines in footer

      // ========= LAST LINE =========
      const lastLineText =
        'NUMERIC CONTROL – DC DRIVE – AC DRIVE – SERVO MOTOR/ DRIVE – ENCODER – TACHO';

      doc
        .font('Roboto-Bold')
        .fontSize(9)
        .fillColor(black)
        .text(lastLineText, margin, rowY, {
          width: innerW,
          align: 'center',
        });
  ...
  /**
   * generateBuffer(data)
   * Returns: Promise<Buffer> that resolves to the full PDF bytes.
   * Use this when you need the PDF in memory before taking further actions (e.g. DB transaction).
   */
  static generateBuffer(data = {}) {
    return new Promise((resolve, reject) => {
      try {
        if (!data.challan_no) return reject(new Error('challan_no required'));
        ...
        // build PDF content (reuse same layout as generate)
        try {
          const pageW = doc.page.width;
          const margin = doc.page.margins.left;
          const innerW = pageW - margin * 2;
          ...
          let rowY = headerY + 20;
          const rowMinH = 22;
          const itemsList = Array.isArray(data.items) && data.items.length ? data.items : [{ description: '', qty: '', remarks: '' }];
          ...
          itemsList.forEach((it, idx) => {
            ...
            const descH = doc.heightOfString(desc || ' ', { width: colDescW - 8, align: 'left' });
            const cellH = Math.max(rowMinH, descH + 6);

            doc.rect(tableX, rowY, tableW, cellH).lineWidth(0.8).strokeColor(black).stroke();
            doc.text(String(idx + 1), tableX + 4, rowY + 4, { width: colNoW - 8, align: 'center' });
            doc.text(desc, tableX + colNoW + 4, rowY + 4, { width: colDescW - 8, align: 'left' });
            doc.text(qty, tableX + colNoW + colDescW + 4, rowY + 4, { width: colQtyW - 8, align: 'center' });
            doc.text(remarks, tableX + colNoW + colDescW + colQtyW + 4, rowY + 4, { width: colRemarksW - 8, align: 'left' });

            rowY += cellH;
          });

          rowY += 10;
          const belowTableText = 'Any descrepancy or rejection should be intimated to us within 2 days of receipt of good, failing which no complaints will be entertained. Guarantee of goods will be void if two copies of delivery challan are not duly signed and returned immediately on reciept of goods.';
          ...
          rowY = signY + 40;
          const factoryText = 'Registered Office & Factory: 20-21, New DLF Industrial Area, Faridabad-121003 Phone: (0129) 4072336 E-mail : infocompageautomtion@gmail.com';
