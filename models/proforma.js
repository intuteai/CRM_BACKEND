// models/proforma.js
// Proforma generator: outer-box layout, GST between lines, meta rows, no footer
// Updated: outer box now ends below the magenta product line (not always to bottom of page).
...
function safeExists(p) { try { return !!(p && fs.existsSync(p)); } catch (e) { return false; } }
function registerFonts(doc) {
  ...
  } catch (e) { logger.warn('Font register failed: ' + e.message); doc.font('Helvetica'); }
}
function withFont(doc, font, size, fn) { doc.save(); if (font) doc.font(font); if (size) doc.fontSize(size); const r = fn(); doc.restore(); return r; }
...
/**
 * drawOuterBorderAndHeader
 * Draws top portion: title, company name/address, top & bottom rules.
 * IMPORTANT: it DOES NOT draw the full outer rect anymore.
 * It returns coordinates (outerX, outerY, outerW, outerH) for use later
 * so the final outer rectangle can be drawn after the footer is placed.
 */
function drawOuterBorderAndHeader(doc) {
  registerFonts(doc);
  ...
  const outerW = pageW - doc.page.margins.left - doc.page.margins.right;
  // outerH is the full area if needed; final height will be computed later
  const outerH = doc.page.height - doc.page.margins.top - doc.page.margins.bottom;

  const magenta = '#e400c8';
  const green = '#00b200';
  const black = '#000000';

  doc.font('Roboto-Bold').fontSize(16).fillColor(magenta);
  doc.text(COMPANY_NAME, 0, outerY + 6, { width: pageW, align: 'center' });

  doc.font('Roboto').fontSize(9).fillColor(green);
  doc.text(COMPANY_ADDR, 0, outerY + 28, { width: pageW, align: 'center' });

  const topRuleY = outerY + 48;
  doc.save();
  ...
  const bottomRuleY = gstY + 14;
  doc.save();
  doc.strokeColor(black).lineWidth(1).moveTo(outerX, bottomRuleY).lineTo(outerX + outerW, bottomRuleY).stroke();
  doc.restore();

  // DO NOT draw the full outer rect here - it will be drawn later once footer position is known.
  // doc.save();
  // doc.lineWidth(1).strokeColor(black).rect(outerX, outerY, outerW, outerH).stroke();
  // doc.restore();
  ...
  return {
    ...
    outerX, outerY, outerW, outerH, innerPad
  };
}
...
class Proforma {
  static generate(data = {}) {
    ...
    registerFonts(doc);
    // header/top strip on each page
    doc.on('pageAdded', () => { drawOuterBorderAndHeader(doc); registerFonts(doc); });

    try {
      // initial layout
      let headerLayout = drawOuterBorderAndHeader(doc);
      let contentLeft = headerLayout.contentLeft;
      let contentTop = headerLayout.contentTop;
      let contentWidth = headerLayout.contentWidth;
      const innerPad = headerLayout.innerPad;
      ...
      doc.restore();
      doc.font('Roboto-Bold').fontSize(12).fillColor(black).text('PROFORMA INVOICE', contentLeft, barY + 3, { width: contentWidth, align: 'center' });

      // To & meta
      const metaTop = barY + barH + 8;
      ...
      function newPageAndHeader() {
        doc.addPage();
        headerLayout = drawOuterBorderAndHeader(doc);
        ...
        doc.font('Roboto-Bold').fontSize(12).fillColor(black).text('PROFORMA INVOICE', contentLeft, barY + 3, { width: contentWidth, align: 'center' });
        // redraw To/meta
        const metaTop2 = barY + barH + 8;
        doc.font('Roboto-Bold').fontSize(10).fillColor(black).text('To,', contentLeft, metaTop2, { width: leftW });
        doc.font('Roboto').fontSize(9).fillColor(black).text(`${toName}${toAddress ? '\n' + toAddress : ''}${toGST}`, contentLeft + 18, metaTop2, { width: leftW - 18 });
        doc.rect(metaX, metaTop2, metaW, metaBoxH).strokeColor('#000').lineWidth(1).stroke();
        for (let i = 1; i < rows; i++) {
          const lineY = metaTop2 + rowHeight * i;
          doc.moveTo(metaX, lineY).lineTo(metaX + metaW, lineY).stroke();
        }
        let rY = metaTop2 + 4;
        doc.font('Roboto').fontSize(9).text(`Proforma Invoice No.: ${data.proforma_no}`, metaX + 6, rY, { width: metaW - 12, align: 'left' });
        rY += rowHeight;
        doc.text(`Date: ${dateStr}`, metaX + 6, rY, { width: metaW - 12, align: 'left' });
        rY += rowHeight;
        doc.text(`Order No.: ${data.order_no || ''}`, metaX + 6, rY, { width: metaW - 12, align: 'left' });
        rY += rowHeight;
        doc.text(`Date: ${dateStr2 || ''}`, metaX + 6, rY, { width: metaW - 12, align: 'left' });

        y = metaTop2 + metaBoxH + 12;
        iaW = contentWidth;
        headerObj = drawItemsHeader(contentLeft, y - 6, iaW);
      ...
      // Render rows with conservative reserved space
      ...
      for (let ri = 0; ri < rowsToProcess.length; ri++) {
        ...
        const reservedForTrailingBlocks = taxAreaH + amountWordsH + rtgsHfinal + sigAreaH + 80;
        const safeBottomForRows = pageBottomLimit() - reservedForTrailingBlocks;

        if (rowCursorY + rh + 6 > safeBottomForRows) {
          newPageAndHeader();
        }
        ...
        doc.moveTo(colX.desc, headerBottom - 6 + (rowCursorY - (headerBottom + 6))).lineTo(colX.desc, rowCursorY + rh + 2).stroke();
      ...
      // Totals and amount-in-words (same behavior as before; no vertical separator)
      let taxBoxY = rowCursorY + 6;
      const trailingNeeded = taxAreaH + amountWordsH + rtgsHfinal + sigAreaH + 40;
      if (taxBoxY + trailingNeeded > doc.page.height - doc.page.margins.bottom) {
        newPageAndHeader();
        taxBoxY = rowCursorY + 6;
      }
      ...
      const minTotalsArea = 240;
      let totalsAreaWidth = Math.max(minTotalsArea, Math.ceil(maxLabelW + labelPadding + numericColumnDesired + 24));
      const maxAllowed = Math.floor(itemsAreaW * 0.45);
      if (totalsAreaWidth > maxAllowed) totalsAreaWidth = maxAllowed;

      const totalsX = baseX + iaW - totalsAreaWidth - 6;
      const taxBoxX = baseX + 6;
      const totalsNumberWidth = Math.min(160, numericColumnDesired);
      const totalsLabelWidth = totalsAreaWidth - totalsNumberWidth - 16;

      const totalRows = [
        { label: 'Taxable Amount', bold: false },
        { label: `Tax @ ${gstPercent}%`, bold: false },
        { label: 'Total Payable', bold: true }
      ];
      let tyEstimate = 0;
      const totalsRowHeights = [];
      for (const r of totalRows) {
        const h = r.bold ? 18 : 16;
        totalsRowHeights.push(h + 6);
        tyEstimate += h + 6;
      }
      const totalsBoxInnerH = tyEstimate + 12;
      const taxBoxH = Math.max(110 - 12, totalsBoxInnerH);
      const totalsAreaHeight = taxBoxH;
      const taxBoxW = Math.max( Math.floor((iaW - totalsAreaWidth - 24)), Math.floor(iaW * 0.50) - 12 );

      doc.rect(taxBoxX, taxBoxY, taxBoxW, taxBoxH).strokeColor('#000').lineWidth(0.8).stroke();
      doc.font('Roboto-Bold').fontSize(12).fillColor('#000')
         .text(`Tax-${data.gst_percent != null ? data.gst_percent + '%' : '18%'}`, taxBoxX + 12, taxBoxY + 28, { width: taxBoxW - 24, align: 'left' });

      doc.rect(totalsX, taxBoxY, totalsAreaWidth, totalsAreaHeight).strokeColor('#000').lineWidth(0.8).stroke();
      ...
      function drawTotalsRow(label, value, opts = {}) {
        ...
        const numW = totalsNumberWidth;
        doc.font(bold ? 'Roboto-Bold' : 'Roboto').fontSize(bold ? 12 : 10);
        doc.text(formatINR(value), numX, ty, { width: numW, align: 'right' });

        const sepY = ty + (bold ? 16 : 14);
        doc.save();
        doc.moveTo(totalsX + 6, sepY).lineTo(totalsX + totalsAreaWidth - 6, sepY).strokeColor('#dcdcdc').lineWidth(0.6).stroke();
        doc.restore();
      ...
      drawTotalsRow('Total Payable', grandTotal, { bold: true, gap: 10 });

      // Amount in words box
      let wordsY = taxBoxY + taxBoxH + 12;
      const wordsBoxH = amountWordsH;
      if (wordsY + wordsBoxH > doc.page.height - doc.page.margins.bottom) {
        newPageAndHeader();
        wordsY = metaTop + metaBoxH + 12;
      }
      ...
      // RTGS block
      let rtgsY = wordsY + 6;
      if (rtgsY + rtgsHfinal > doc.page.height - doc.page.margins.bottom) {
        newPageAndHeader();
        rtgsY = metaTop + metaBoxH + 12; // start after meta on new page
      }
      const rtgsStartX = baseX;
      const rtgsW = iaW;
      doc.rect(rtgsStartX, rtgsY, rtgsW, rtgsHfinal).strokeColor('#000').lineWidth(0.8).stroke();
      doc.font('Roboto-Bold').fontSize(9).text('RTGS Detail:', rtgsStartX + 6, rtgsY + 6, { width: 200, align: 'left' });
      const rtgsRowY = rtgsY + 22;
      const cW = Math.floor((rtgsW - 12) / 4);
      const x0 = rtgsStartX + 6;
      doc.font('Roboto-Bold').fontSize(8).text('ACCOUNT NUMBER', x0, rtgsRowY, { width: cW, align: 'left' });
      doc.text('BANK NAME', x0 + cW, rtgsRowY, { width: cW, align: 'left' });
      doc.text('NAME OF THE BRANCH', x0 + cW * 2, rtgsRowY, { width: cW, align: 'left' });
      doc.text('BRANCH RTGS CODE', x0 + cW * 3, rtgsRowY, { width: cW, align: 'left' });
      const valY = rtgsRowY + 14;
      doc.font('Roboto').fontSize(9).text(RTGS_ACCOUNT, x0, valY, { width: cW, align: 'left' });
      doc.text(RTGS_BANK, x0 + cW, valY, { width: cW, align: 'left' });
      doc.text(RTGS_BRANCH, x0 + cW * 2, valY, { width: cW, align: 'left' });
      doc.text('IFSC Code:-' + RTGS_IFSC, x0 + cW * 3, valY, { width: cW, align: 'left' });

      // ---------- Signature + Footer: improved page-space check and drawing ----------
      const pageBottom = doc.page.height - doc.page.margins.bottom;
      let sigStartCandidateY = rtgsY + rtgsHfinal + 12;

      const approxSigImgH = 48;
      const sigCaptionGap = 8;
      const captionsHeight = 18;
      const gapAfterCaptions = 18;

      const footerLineThickness = 1.6;
      const factoryFontSize = 11;
      const factoryLineGap = 6;
      const magentaFontSize = 10;
      const magentaLineGap = 6;
      const finalLineGap = 8;

      const neededForSignature = approxSigImgH + sigCaptionGap + captionsHeight;
      const neededForFooter = (footerLineThickness) + factoryFontSize + factoryLineGap + (footerLineThickness) + magentaFontSize + magentaLineGap + finalLineGap;
      const totalNeeded = neededForSignature + gapAfterCaptions + neededForFooter + 20; // breathing

      if (sigStartCandidateY + totalNeeded > pageBottom) {
        newPageAndHeader();
        sigStartCandidateY = metaTop + metaBoxH + 12;
      }
      ...
      const sigImgY = sigStartCandidateY;

      const sigPath = findAsset(ASSETS.signature, 'compage_signature.png');
      if (sigPath && safeExists(sigPath)) {
        try { doc.image(sigPath, sigImgX, sigImgY, { width: sigImgMaxW }); }
        catch (e) { logger.warn('Signature draw failed: ' + e.message); }
      }

      const captionsY = sigImgY + approxSigImgH + sigCaptionGap;

      doc.font('Roboto-Bold').fontSize(11).fillColor('#12a54b');
      doc.text("Receiver's Signature", leftColX, captionsY, { width: leftColW, align: 'left' });

      doc.font('Roboto').fontSize(10).fillColor('#12a54b');
      doc.text(`For ${COMPANY_NAME}`, rightColX, captionsY, { width: rightColW, align: 'right' });

      // Footer drawing
      const footerLineY1 = captionsY + gapAfterCaptions;
      const innerLeft = contentLeft, innerRight = contentLeft + contentWidth;

      doc.save();
      doc.moveTo(innerLeft, footerLineY1).lineTo(innerRight, footerLineY1).strokeColor('#000').lineWidth(footerLineThickness).stroke();
      doc.restore();

      const factoryY = footerLineY1 + 8;
      doc.font('Roboto-Bold').fontSize(factoryFontSize).fillColor('#12a54b');
      const factoryText = 'Factory: 20-21, New DLF Industrial Area, Faridabad-121003, Phone: (0129) 4072336';
      doc.text(factoryText, contentLeft, factoryY, { width: contentWidth, align: 'center' });

      const footerLineY2 = factoryY + 18;
      doc.save();
      doc.moveTo(innerLeft, footerLineY2).lineTo(innerRight, footerLineY2).strokeColor('#000').lineWidth(footerLineThickness).stroke();
      doc.restore();

      const magentaY = footerLineY2 + 8;
      doc.font('Roboto-Bold').fontSize(magentaFontSize).fillColor('#e400c8');
      const magentaText = 'NUMERIC CONTROL – DC DRIVE – AC DRIVE – SERVO MOTOR/ DRIVE – ENCODER – TACHO'.trim();
      doc.text(magentaText, contentLeft, magentaY, { width: contentWidth, align: 'center' });

      const footerLineY3 = magentaY + 18;
      doc.save();
      doc.moveTo(innerLeft, footerLineY3).lineTo(innerRight, footerLineY3).strokeColor('#000').lineWidth(footerLineThickness).stroke();
      doc.restore();

      // -------------------- DRAW FINAL OUTER BOX ENDING AT FOOTER --------------------
      // Use outer coordinates returned earlier by drawOuterBorderAndHeader.
      // outer top is headerLayout.outerY, left is headerLayout.outerX, width is headerLayout.outerW
      // compute height so bottom is just below footerLineY3
      try {
        const outerTop = headerLayout.outerY;
        const outerLeft = headerLayout.outerX;
        const outerWidth = headerLayout.outerW;
        // ensure a small padding below the final line
        const bottomPadding = 8;
        const computedOuterH = Math.max( (footerLineY3 + bottomPadding) - outerTop, 40 );
        doc.save();
        doc.lineWidth(1).strokeColor('#000');
        doc.rect(outerLeft, outerTop, outerWidth, computedOuterH).stroke();
        doc.restore();
      } catch (e) {
        // if anything fails here, fallback: do nothing (we kept header/top already)
        logger.warn('Final outer box draw failed: ' + (e && e.message));
      }
      ...
      try { doc.end(); } catch (e) {}
      logger.error('Proforma generator error:', err);
      throw err;
    }
  }
}
