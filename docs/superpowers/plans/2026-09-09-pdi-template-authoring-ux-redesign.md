...
**3. (Added after Task 2's code-quality review caught this live — read before writing Task 3's editors.) The per-row exclusion-set pattern uses `.add()` to re-introduce sibling keys, never `.delete()`.** Task 2's `HeaderSectionEditor`/`PhotoSectionEditor`/`SignatureSectionEditor` each build a section-level exclusion set by *removing* all of that section's own keys (so the section doesn't collide with itself), then, per row, were supposed to *re-add* every OTHER row's key so sibling rows in the same list could still collide with each other. The first-written version of this file called `.delete()` in that per-row step instead of `.add()` — a no-op, since those keys were already absent from the set — silently allowing two rows in the same list (two photo slots, two signers, two header detail rows) to receive the identical auto-generated key, which then silently overwrite each other in the flattened fill-out data. This was caught by code-quality review and fixed in commit `4767113` (see the corrected `HeaderSectionEditor`/`PhotoSectionEditor`/`SignatureSectionEditor` in the actual committed file, not this plan's original Task 2 code block, which still has the bug as originally written). **Task 3's `ChecklistSectionEditor` and `FillInListSectionEditor` use this exact same exclusion-set pattern for their own row/column lists — write them with `.add()` for the sibling re-introduction step from the start, not `.delete()`.** Also fixed in the same commit: `ListEditor`'s drag-drop `insertAt` computation no longer subtracts 1 for downward drags (that subtraction made dropping an item onto its immediate next sibling a no-op) — it now always splices at the unadjusted `dropIndex`, which is correct for both directions. Task 3 doesn't touch `ListEditor` itself, but relies on it, so implementers should pull the current committed version, not re-derive it from this plan's Task 2 text.
