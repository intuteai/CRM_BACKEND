...
class QueryService {
  ...
  static async respondToQuery(queryId, respondedBy, responseText) {
    ...
    try {
      ...
      if (!query) throw new QueryError('Query not found', 404);
      ...
      const customerName = (
        await client.query('SELECT name FROM users WHERE user_id = $1', [updatedQuery.user_id])
      ).rows[0]?.name || 'Unknown User';
      await client.query('COMMIT');
